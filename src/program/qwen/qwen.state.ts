// src/program/qwen/qwen.state.ts
...
@Injectable()
export class QwenState {
  private readonly memberId = 'qwen-max';
  ...
  async reply(chatId: string) {
    try {
      ...
      if (response.choices.length > 0) {
        ...
        throw new Error('No content received from Aliyun.');
      }
    } catch (error) {
      console.error('Error calling Aliyun:', error);
      throw new Error('Failed to get response from Aliyun.');
    } finally {
