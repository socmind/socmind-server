// src/program/kimi/kimi.state.ts
...
@Injectable()
export class KimiState {
  private readonly memberId = 'kimi-latest';
  ...
  async reply(chatId: string) {
    try {
      ...
      if (response.choices.length > 0) {
        ...
        throw new Error('No content received from Moonshot.');
      }
    } catch (error) {
      console.error('Error calling Moonshot:', error);
      throw new Error('Failed to get response from Moonshot.');
    } finally {
